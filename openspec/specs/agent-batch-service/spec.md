# agent-batch-service Specification

## Purpose

Servicio Python del agente para lanzar y consultar jobs del rag-indexer: `BatchService` (HTTP en localstack, AWS Batch en producción) y `RagIndexerBatchTrigger` (`trigger_full` / `trigger_delta`).
## Requirements
